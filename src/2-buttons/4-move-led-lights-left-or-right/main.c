...
void main(void)
{
    unsigned char ledValue, keyNum, carry;

    /**
     * The main idea of this code is to scan for pressed buttons on the 8 buttons
     * keyboard. Then, if K1 is pressed, shift the only LED turned on to the left.
     * But if K2 is pressed, then shift the only LED turned on to the right.
     */
    ...
    while (1)
    {
        ...
        if (keyNum == 0xFE) /* if K1 is pressed */
        {
            carry = ledValue & 0x80 ? 1 : 0;
		    ledValue = (ledValue << 1) | carry; /* shift LED to the left, with carry */
        }
        else if (keyNum == 0xFD) /* if K2 is pressed */
        {
            carry = ledValue & 1 ? 0x80 : 0;
            ledValue = (ledValue >> 1) | carry; /* shift LED to the right, with carry */
        }
